...
the ProgressLogger gem doesn't do much - it just tries to handle the interval checking, leaving what you do every
...
You can do lots of other stuff too - see the ProgressLogger rdocs for more, but an example to whet your appetite:
    p = ProgressLogger.new(:count => 1000000, :minutes => 30, :max => @db.size) do |state|
      @db.flush()
      @logger.info("Processed #{state.count} rows - #{state.long_eta/(60*60.0)} hours to go!")
    end
    @db.find_all.each do |record|
      p.trigger
      # do stuff
